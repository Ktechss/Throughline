...
export default function NailPicker({ nails = [], selected, onSelect, onClear, onSave, onDelete }) {
  ...
  return (
    <div>
        ...
        <div className="flex flex-wrap gap-1 mb-2 p-1">
          {categories.map((c) => (
            <button key={c} onClick={() => setCat(c)}
              className={cn("rounded-full px-2.5 py-1 text-[10px] whitespace-nowrap ring-1 transition-colors", cat === c ? "bg-white/10 text-white ring-white/15" : "text-zinc-400 ring-line hover:text-zinc-200")}>
      ...
      {selected && (
        <div className="mb-3 rounded-lg ring-1 ring-line bg-surface px-2.5 py-2">
          <div className="text-[11px] font-medium text-zinc-200">{selected.name}{selected.category && selected.category !== "Uncategorized" && <span className="ml-2 text-[11px] text-zinc-500">{selected.category}</span>}</div>
      ...
      <div className="grid gap-2 grid-cols-[repeat(auto-fill,minmax(72px,1fr))]">
        <button onClick={() => onClear?.()}
          className={cn("flex aspect-square flex-col items-center justify-center gap-1 rounded-lg ring-1 text-[10px] transition-colors",
            !selected ? "ring-2 ring-emerald-400 bg-emerald-500/10 text-white" : "ring-line-subtle text-zinc-500 hover:text-zinc-300 hover:ring-white/25")}>
        ...
        {shown.map((n) => (
          <div key={n.id} title={n.name}
            className={cn("group relative aspect-square rounded-lg overflow-hidden ring-1 cursor-pointer transition-all",
              selected?.id === n.id ? "ring-2 ring-emerald-400" : "ring-line-subtle hover:ring-white/25")}
            onClick={() => onSelect?.(n)}>
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-1 pt-2 pb-0.5">
              <span className="block truncate text-[11px] text-zinc-200">{n.name}</span>
            </div>
