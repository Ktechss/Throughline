...
export default function Shoot({
  ...
  poseId, setPoseId, poseText, setPoseText, poseLibrary, resolution, setResolution, faceAcc, setFaceAcc,
      ...
      <section className="eve-panel space-y-5">
        ...
        <div>
          <div className="mb-2 flex items-center gap-2">
            ...
            {(poseId || poseText) && <button onClick={() => { setPoseId(''); setPoseText?.('') }} className="ml-auto text-[10px] text-[#8a8a99] hover:text-[#e6e6ea]">clear</button>}
          </div>

          {/* Reused pose from an older shot whose id no longer exists in the library. */}
          {poseText && !poseId && (
            <div className="mb-3 rounded-lg border border-[#315d88] bg-[#101b27] p-3">
              <p className="text-[11px] font-medium text-[#8fb6dd]">reusing a saved pose from an earlier shot</p>
              <p className="mt-1 text-[11px] leading-relaxed text-[#8a8a99]">{poseText}</p>
              <p className="mt-1 text-[10px] text-[#5f6b7a]">its original pose is no longer in the library — the exact text is applied instead. Pick a pose below to replace it.</p>
            </div>
          )}
