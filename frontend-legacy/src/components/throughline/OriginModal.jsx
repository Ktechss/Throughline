...
import { ep, runView } from '@/lib/throughline'
            ...
            <>
              ...
              <div className="mt-2 flex items-center gap-3 rounded-lg border border-[#24242e] bg-[#0e0e12] p-3">
                ...
                <div className="min-w-0 flex-1">
                  {m.pose_id
                    ? <p className="text-sm text-[#e6e6ea]">{m.pose_id}{pose?.category
                        ? <span className="ml-2 text-[10px] text-[#5f6b7a]">{pose.category}</span>
                        : <span className="ml-2 text-[10px] text-[#5f6b7a]">not in current library</span>}</p>
                    : <p className="text-sm text-[#e6e6ea]">custom pose reference</p>}
                  {(pose?.text || m.pose_text) && <p className="mt-0.5 text-[11px] leading-relaxed text-[#8a8a99]">{pose?.text || m.pose_text}</p>}
